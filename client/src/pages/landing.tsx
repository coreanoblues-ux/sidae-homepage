...
export default function Landing() {
  ...
  // 프로그램 상세 내용 (데스크톱 하단 패널 / 모바일 카드 아래 인라인 패널에서 공통 사용)
  const renderProgramDetail = (program: 'middle' | 'high' | 'gifted') => (
    <>
      {/* ── 중등부 상세 ── */}
      {program === 'middle' && (
      ...
      <section id="programs" className="py-20 bg-muted/30 pattern-bg-alt scroll-mt-20">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-4">
            ...
            {/* 모바일/태블릿: 중등부 카드 바로 아래 상세 (데스크톱은 하단 패널 사용) */}
            {expandedProgram === 'middle' && (
              <div className="col-span-full lg:hidden rounded-2xl border border-primary/30 bg-background shadow-xl overflow-hidden">
                {renderProgramDetail('middle')}
              </div>
            )}
            ...
            {/* 모바일/태블릿: 고등부 카드 바로 아래 상세 (데스크톱은 하단 패널 사용) */}
            {expandedProgram === 'high' && (
              <div className="col-span-full lg:hidden rounded-2xl border border-primary/30 bg-background shadow-xl overflow-hidden">
                {renderProgramDetail('high')}
              </div>
            )}
            ...
            {/* 모바일/태블릿: 영재고 카드 바로 아래 상세 (데스크톱은 하단 패널 사용) */}
            {expandedProgram === 'gifted' && (
              <div className="col-span-full lg:hidden rounded-2xl border border-primary/30 bg-background shadow-xl overflow-hidden">
                {renderProgramDetail('gifted')}
              </div>
            )}
          </div>

          {/* ── 펼쳐지는 상세 내용 패널 (데스크톱 전용: 그리드 아래 전체 폭) ── */}
          {expandedProgram && (
            <div className="hidden lg:block mt-6 rounded-2xl border border-primary/30 bg-background shadow-xl overflow-hidden">
              {renderProgramDetail(expandedProgram)}
            </div>
          )}
